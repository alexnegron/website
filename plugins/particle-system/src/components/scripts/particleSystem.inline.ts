...
const initializeParticleSystem = () => {
  ...
  const animate = (time: number) => {
    ...
    const step = Math.max(0, Math.min((time - lastTime) / (1000 / 60), 2))
    lastTime = time
    evolve(step)
    draw()
  ...
  window.addCleanup(disposeParticleSystem)
}

document.addEventListener("nav", initializeParticleSystem)
document.addEventListener("render", initializeParticleSystem)
